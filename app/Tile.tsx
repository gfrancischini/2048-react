...
export const Tile = (props: ITileProps) => {
    ...
    const convertTilePositionToPixels = (x: number, y: number) => {
        return {
            left: x * (dimensions.tile.width + dimensions.tile.margin * 2) + dimensions.tile.margin * 2,
            top: y * (dimensions.tile.height + dimensions.tile.margin * 2) + dimensions.tile.margin * 2,
        }
    }
    ...
    const scaleAnimated = useRef(new Animated.Value(tile.previousPosition ? 1 : 0)).current;

    // the current render pixel positions (this is need to play animation correctly)
    const [renderPixelPosition, setRenderPixelPosition] = useState(convertTilePositionToPixels(tile.previousPosition?.x ?? tile.position.x, tile.previousPosition?.y ?? tile.position.y));

    useEffect(() => {
        if (tile.previousPosition != null) {
            // if we have a previous position we will play the move navigation
            const previousPixelPosition = convertTilePositionToPixels(tile.previousPosition.x, tile.previousPosition.y);
            const newPixelsPosition = convertTilePositionToPixels(tile.getPosition().x, tile.getPosition().y);
    ...
    const tileStyle = tile.value <= 2048 ? styles['tile' + tile.value] : styles['tilesuper']
    const tileTextStyle = tile.value <= 2048 ? styles['tile' + tile.value + 'Text'] : styles['tilesuperText']
