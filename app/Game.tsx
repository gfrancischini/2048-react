...
export const Game = () => {
  ...
  const [showModal, setShowModal] = useState<'HELP' | 'WON' | 'OVER' | null>(
    null,
  );
  ...
  const actuateOnState = () => {
    ...
    saveData();
    setShowModal(
      gameManager.won && !gameManager.shouldKeepPlaying
        ? 'WON'
        : gameManager.over
        ? 'OVER'
        : null,
  ...
  const onGameWonModalDismiss = (
    action: 'KEEP_PLAYING' | 'RESTART' | 'NONE',
  ) => {
    setShowModal(null);
    switch (action) {
      case 'KEEP_PLAYING':
    ...
    <>
      {showModal === 'WON' ||
        (showModal === 'OVER' && (
          <GameWonOverModal
            type={showModal}
            onModalDismiss={onGameWonModalDismiss}
          />
        ))}
